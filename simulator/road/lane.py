...
from simulator.road.spline import LinearSpline2D
from simulator.utils import Vector, class_from_path, get_class_path, wrap_to_pi
...
class AbstractLane(object):
    ...
    metaclass__ = ABCMeta
    DEFAULT_WIDTH: float = lane_width
    VEHICLE_LENGTH: float = car_length
    length: float = 0
...
class CircularLane(AbstractLane):
    ...
    def position(self, longitudinal: float, lateral: float) -> np.ndarray:
        ...
        return self.center + (self.radius - lateral * self.direction)*np.array([np.cos(phi), np.sin(phi)]).T
...
class CompositionLane(AbstractLane):
    def __init__(self, lanes) -> None:
        super().__init__()
        self.lanes = lanes
        self.lengths = [lane.length for lane in lanes]
        self.line_types = [LineType.NONE, LineType.NONE]
    
    def get_lane_longitudinal(self, longitudinal):
        for i, (lane, L) in enumerate(zip(self.lanes, self.lengths)):
            if longitudinal < L:
                return lane, longitudinal
            longitudinal -= L
        return lane, longitudinal + L # Exceeded last lane

    def position(self, longitudinal: float, lateral: float) -> np.ndarray:
        lane, longitudinal = self.get_lane_longitudinal(longitudinal)
        return lane.position(longitudinal, lateral)

    def heading_at(self, longitudinal: float) -> float:
        lane, longitudinal = self.get_lane_longitudinal(longitudinal)
        return lane.heading_at(longitudinal)

    def width_at(self, longitudinal: float) -> float:
        lane, longitudinal = self.get_lane_longitudinal(longitudinal)
        return lane.width_at(longitudinal)

    def local_coordinates(self, position: np.ndarray) -> Tuple[float, float]:
        positions = []
        offset = 0
        for lane, length in zip(self.lanes, self.lengths):
            long, lat = lane.local_coordinates(position)
            positions.append((long + offset, lat))
            offset += length
        return min(positions, key=lambda long_lat: abs(long_lat[1]))

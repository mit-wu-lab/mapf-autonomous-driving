...
import numpy as np
import pygame
from simulator.road.lane import AbstractLane, LineType
from simulator.road.road import Road
from simulator.utils import Vector
from simulator.vehicle.graphics import VehicleGraphics
from simulator.vehicle.objects import Landmark, Obstacle

if TYPE_CHECKING:
    from vehicle.objects import RoadObject
...
class RoadGraphics(object):
    ...
    @staticmethod
    def display_subzones(subzones, surface: WorldSurface) -> None:
        for subzone_id, subzone in subzones.items():
            if (vehicle := subzone.get('reserving_vehicle', None)) is not None:
                pygame.draw.polygon(surface, subzone.reserving_vehicle.color, [surface.pos2pix(x, y) for x, y in subzone.shape], 0)
        for subzone_id, subzone in subzones.items():
            pygame.draw.polygon(surface, (60, 60, 60), [surface.pos2pix(x, y) for x, y in subzone.shape], 2)

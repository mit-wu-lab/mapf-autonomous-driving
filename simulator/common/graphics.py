...
import numpy as np
import pygame
from simulator.common.action import ActionType, ContinuousAction
from simulator.road.graphics import RoadGraphics, WorldSurface
from simulator.vehicle.graphics import VehicleGraphics
...
class EnvViewer(object):
    ...
    def __init__(self, env, config: Optional[dict] = None) -> None:
        ...
        self.frame = 0
        self.frames_dir = (run_dir / 'frames').mk() if render_images else None

        pygame.init()
        pygame.display.set_caption("Simulator_v_1.0")
    ...
    def set_agent_action_sequence(self, actions) -> None:
        """
    ...
    def display(self) -> None:
        ...
        RoadGraphics.display_subzones(self.env.subzones, self.sim_surface)
        ...
        if self.frames_dir:
            pygame.image.save(self.sim_surface, f'{self.frames_dir}/highway-env_{self.frame:04d}.png')
            self.frame += 1
...
class EventHandler(object):
    ...
    def handle_event(cls, action_type: ActionType, event: pygame.event.EventType) -> None:
        ...
        if action_type.__class__ == ContinuousAction:
            cls.handle_continuous_action_event(action_type, event)

    @classmethod
    def handle_discrete_action_event(cls, action_type, event: pygame.event.EventType) -> None:

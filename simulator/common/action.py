...
from simulator.vehicle.controller import AccOnlyVehicle
# from vehicle.behavior import IDMVehicle
# from vehicle.dynamics import BicycleVehicle
from simulator.vehicle.kinematics import Vehicle

# from vehicle.controller import MDPVehicle
...
class ContinuousAction(ActionType):
    ...
    def act(self, action: np.ndarray) -> None:
        ...
        if self.longitudinal and self.lateral:
            ...
            # self.controlled_vehicle.act({"acceleration": action[0], "steering": action[1]})
...
class AccOnlyAction(ActionType):
    ...
    def space(self) -> spaces.Box:
        return spaces.Box(-12., 12., shape=(1,), dtype=np.float32)

    @property
    def vehicle_class(self) -> Callable:
        return AccOnlyVehicle
...
def action_factory(env, config: dict) -> ActionType:
    ...
    elif config["type"] == "AccOnlyAction":
        return AccOnlyAction(env, **config)

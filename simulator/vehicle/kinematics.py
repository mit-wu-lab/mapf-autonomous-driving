...
from simulator.road.road import LaneIndex, Road
from simulator.utils import Vector
from simulator.vehicle.objects import Landmark, Obstacle, RoadObject
...
class Vehicle(RoadObject):
    ...
    def __init__(self,
        ...
        self.longitudinal, _ = self.road.network.get_lane(self.target_lane_index).local_coordinates(self.position)
    ...
    def step(self, dt: float) -> None:
        ...
        self.heading += self.speed * np.tan(delta_f) * np.cos(beta) / self.LENGTH * dt
    ...
    def plan_route_to(self, destination: str) -> "Vehicle":
        """
        ...
        try:
            ...
            path = []
        if path:
            self.HE_route = [self.lane_index] + [(path[i], path[i + 1], None) for i in range(len(path) - 1)]
        else:
            self.HE_route = [self.lane_index]
        return self

    def follow_road(self) -> None:
        """At the end of a lane, automatically switch to a next one."""
        prev_longitudinal = self.longitudinal
        curr_lane = self.road.network.get_lane(self.target_lane_index)
        curr_longitudinal, _ = curr_lane.local_coordinates(self.position)
        if curr_longitudinal < prev_longitudinal or curr_lane.after_end(self.position, curr_longitudinal):
            self.target_lane_index = self.road.network.next_lane(self.target_lane_index, route=self.HE_route, position=self.position, np_random=self.road.np_random)
            curr_longitudinal, _ = self.road.network.get_lane(self.target_lane_index).local_coordinates(self.position)
        self.longitudinal = curr_longitudinal